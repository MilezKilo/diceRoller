...
def throwDicesVarThree():
    ...
    if 'd' in list(dices[1]):  # Простая проверка ввода
        if '+' in dices or '-' in dices:  # Даннный кейс выполняется если есть доп число с знаком - или +
            innerValue = dices.split('d')
            ...
            if '+' in dices:
                dividedEdgesAndAdditionalNumber = inputedEdges.split("+")  # Разделяем количество граней и дополнительное положительное число число
                ...
                dividedEdgesAndAdditionalNumber = inputedEdges.split("-")  # Разделяем количество граней и дополнительное отрицательное число число
                additionalNumber = int(dividedEdgesAndAdditionalNumber[-1])  # Дополнительное число
                y -= additionalNumber  # Прибавляем дополнительное число к у
                lastDigit = '-' + str(additionalNumber)  # Преобразовываем доп число в "+ДопЧисло"
